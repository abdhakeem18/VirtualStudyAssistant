...
export default function SignupForm({ navigation }) {
  ...
  const handleResendEmail = async () => {
    ...
    try {
      const apiv = API("v1");
      const response = await apiv.post("/auth/resend-confirmation", {
      ...
      {!waitingForEmail ? (
        ...
        <>
          ...
          <Text className="text-xs text-gray-500 mb-2 text-center">
            If you did not receive the email, you can resend the confirmation
            link after 5 minutes.
          </Text>
          <Button
            name={
              resendAvailable
                ? "Resend Confirmation Email"
                : "Resend available in 5 min"
            }
            callback={handleResendEmail}
            btnCls={
              resendAvailable
                ? "bg-purple-950 p-3 rounded-2xl mb-3"
                : "bg-gray-400 p-3 rounded-2xl mb-3"
            }
